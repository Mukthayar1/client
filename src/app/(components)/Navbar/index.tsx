    ...
    <div className="flex items-center justify-between bg-white px-4 py-3 dark:bg-black dark:px-4 dark:py-3">
      ...
      <div className="flex items-center gap-8">
        {!isSidebarCollapsed ? null : (
          <button onClick={() => dispatch(setSidebarCollapsed(true))}>
            <MenuIcon className="h-8 w-8 dark:text-white" />
          </button>
        )}

        <div className="relative flex h-min w-[200px]">
          <Search className="absolute top-1/2 left-[4px] mr-2 w-5 -translate-y-1/2 transform cursor-pointer dark:text-white" />
          <input
      ...
      <button
        ...
        className={
          isDarkMode
            ...
            : "h-min w-min rounded p-2 dark:bg-gray-100"
        }
      >
        {isDarkMode ? (
